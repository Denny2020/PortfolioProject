...
-- Shows likelihood of dying if you contract covid in Austrial
...
Select dea.continent, dea.location, Trim(dea.date), dea.population, vac.new_vaccinations
,SUM(CONVERT(vac.new_vaccinations, double)) OVER (Partition by dea.location Order by dea.location, dea.Date) as RollingPeopleVaccinated
-- ,(RollingPeopleVaccinated/population)*100
From PortfolioProjects.coviddeaths dea
Join PortfolioProjects.covidVaccinations vac
...
Create or Replace View PercentPopulationVaccinated 
AS
Select dea.continent, dea.location, Trim(dea.date), dea.population, vac.new_vaccinations
,SUM(CONVERT(vac.new_vaccinations,double)) OVER (Partition by dea.Location Order by dea.location, dea.Date) as RollingPeopleVaccinated
From PortfolioProjects.coviddeaths dea
